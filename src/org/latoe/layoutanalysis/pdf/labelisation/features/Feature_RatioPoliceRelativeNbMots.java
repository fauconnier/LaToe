...
public class Feature_RatioPoliceRelativeNbMots extends FeatureTypes {
	/**
	 * 
	 */
	...
	public Feature_RatioPoliceRelativeNbMots(FeatureGenImpl m, float borneInf, float borneSup) {
		super(m);
		numStates = m.numStates();
		fname = "RatioPoliceRelativeSurNbMotsEntre"+borneInf+"et"+borneSup;
                this.bInf = borneInf;
                this.bSup = borneSup;
	}
	...
	public boolean startScanFeaturesAt(DataSequence data, int prevPos, int pos) {
                ...
                float pourcentageDiffPolice = (currChunk.modeTaillePolice - d.getModeTaillePolice())*(float)(d.getModeTaillePolice());
//                System.out.println(currChunk.type + "\t" + Float.toString(pourcentageDiffPolice/currChunk.mots.size()) + "\t pourcent diff=\t"+Float.toString(pourcentageDiffPolice)+"\tnb mots=\t"+currChunk.mots.size()+"\t mode doc=\t"+d.getModeTaillePolice()+"\t mode chunk =\t"+currChunk.modeTaillePolice);
                float ratio = pourcentageDiffPolice / currChunk.mots.size();
                if (ratio >= this.bInf && ratio < this.bSup ) {
                    stateId = 0;
                    return true;
                } 
                else{
