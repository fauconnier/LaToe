...
public class Feature_MoinsMargeHautMoy extends FeatureTypes {
	String fname;
	int stateId;
	int numStates;
	String nameFeatures;
	...
	public Feature_MoinsMargeHautMoy(FeatureGenImpl m) {
		super(m);
		numStates = m.numStates();
		fname = "MoinsMargeHaut";
		this.value=value;
	}
	...
	public boolean startScanFeaturesAt(DataSequence data, int prevPos, int pos) {
		...
		Chunk_PDF currChunk = (Chunk_PDF) d.x(pos);
                ...
                if(!currChunk.isFirstOnPage){
                    ...
                    if (margeHauteChunk > 0 && (margeHauteChunk < (margeHauteDocu * (1 - FeatureGenerator_Labelisation.seuil)))) {
                                stateId = 0;
                                return true;
