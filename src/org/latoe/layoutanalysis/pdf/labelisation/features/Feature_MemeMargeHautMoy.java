...
public class Feature_MemeMargeHautMoy extends FeatureTypes {
	String fname;
	int stateId;
	int numStates;
	String nameFeatures;
	...
	public Feature_MemeMargeHautMoy(FeatureGenImpl m) {
		super(m);
		...
		Chunk_PDF currChunk = (Chunk_PDF) d.x(pos);
                ...
                if(!currChunk.isFirstOnPage){
                    ...
                    if (margeHauteChunk > (margeHauteDocu * (1 - FeatureGenerator_Labelisation.seuil)) && 
                            margeHauteChunk < (margeHauteDocu * (1 + FeatureGenerator_Labelisation.seuil))) {
                                stateId = 0;
                                return true;
