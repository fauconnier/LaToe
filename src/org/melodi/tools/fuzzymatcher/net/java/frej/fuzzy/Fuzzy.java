...
package org.melodi.tools.fuzzymatcher.net.java.frej.fuzzy;
 ...
 * string matching or fuzzy substring search.
 * 
 * Based on Demerau-Levenshtein distance evaluation, i.e. there are four types
 * of "mistakes" each counting as 1 point (char deletion, char adding, char
 * replacement, swap of two adjacent chars).
 * 
 * Methods are , and resulting variables too, so necessary values should be read
 * before new matching/searching attempt.
 * 
...
public class Fuzzy {
	...
	/**
	 * if result of match is higher than threshold, boolean methods return
	 * "false"
	 */
	...
	int MAX_PATTERN;
	int MAX_SOURCE;
	int BIG_VALUE = 1000000;
	int[][] e = new int[MAX_PATTERN + 1][MAX_SOURCE + 1];
	WayType[][] w = new WayType[MAX_PATTERN + 1][MAX_SOURCE + 1];
	...
	public Fuzzy(int lengthsource, int lengthpattern) {
		this.MAX_PATTERN = lengthpattern;
		this.MAX_SOURCE = lengthsource;
		this.e = new int[MAX_PATTERN + 1][MAX_SOURCE + 1];
		this.w = new WayType[MAX_PATTERN + 1][MAX_SOURCE + 1];
	}
	 ...
	 * 
	 * @return position of found substring end (0 .. source.length() - 1) or
	 *         (-1) if substring was not found (with given threshold).
	 */
	 ...
	 * 
	 * @return true or false depending on whether any of pattern search
	 *         succeeds.
	 */
	...
	public double containability(CharSequence source, CharSequence pattern) {
		...
		for (int x = 0; x < n; x++) {
//			System.out.println("Attribution ... " + x + "/" + n);
			e[0][x] = 0;
		} // for
...
//			System.out.println("Search ... " + y + "/" + m);
	...
	public double bestEqual(String string, Object patterns, boolean equality) {
		...
		for (int i = 0; i < array.length; i++) {
			double cur = equality ? similarity(string, array[i])
					: containability(string, array[i]);
			if (cur < value) {
				value = cur;
				resultIndex = i;
	...
	/**
	 * Core method for measuring Demerau-Levenshtein distance between two
	 * strings.
	 * 
	 * @return normalized distance (distance / average(source.length(),
	 *         pattern.length()))
	 */
	...
	public void setMAX_PATTERN(int mAX_PATTERN) {
		MAX_PATTERN = mAX_PATTERN;
		e = new int[MAX_PATTERN + 1][MAX_SOURCE + 1];
		WayType[][] w = new WayType[MAX_PATTERN + 1][MAX_SOURCE + 1];
	}

	public int getMAX_SOURCE() {
		return MAX_SOURCE;
	}

	public void setMAX_SOURCE(int mAX_SOURCE) {
