...
public class MediaWikiParserFactory {
	...
	public void initFrenchVariables() throws FileNotFoundException{
	...
	//		templateParserClass = FlushTemplates.class;
			...
			// Identifier images
			imageIdentifers = getParseText(base + "image_identifiers.txt", false, true);
			
			// A enlever
			deleteTemplates = getParseText(base + "delete_templates.txt", false, true);
			
			// Catégorie
			categoryIdentifers =  getParseText(base + "category_identifiers.txt",false, true);
			
			// LanguageIdentifier
			languageIdentifers.remove("fr");
			
			// Toparser
			parseTemplates = getParseText(base + "parser_templates.txt",false, true);

		}

	
	public ArrayList<String> getParseText(String path, boolean print, boolean normalize) throws FileNotFoundException{
		
		ArrayList<String> array_string = new ArrayList<String>();
		...
		while (scanner.hasNextLine()) {
		    ...
		    if(!line.startsWith("#") && line.length() > 0){
		    	...
		    	if(path.contains("parser_templates")){
		    	...
		    	if(normalize){
		    		...
		    		array_string.add(line.toUpperCase());
		    		array_string.add(line.toLowerCase());
		    	}
		    }
...
//					resolveLineSeparator(),
			...
			if( templateParserClass == GermanTemplateParser.class ){
			...
			else if( templateParserClass == FrenchTemplateParser.class ){
				for( String s: deleteTemplates) {
					logger.debug( "DeleteTemplate: '" + s + "'");
				}
				for( String s: parseTemplates) {
				...
				mwtp = new FrenchTemplateParser( mwgp, deleteTemplates, parseTemplates );
			}
